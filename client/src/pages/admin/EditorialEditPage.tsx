...
import { containerTypeEnum, insertEditorialSchema, Editorial } from '@shared/schema';
import { zodResolver } from '@hookform/resolvers/zod';
import { useForm } from 'react-hook-form';
import { z } from 'zod';
import { FileUploader } from '@/components/admin/FileUploader';

// Page d'édition d'un élément éditorial
export function EditorialEditPage() {
  const { id } = useParams<{ id: string }>();
  const queryClient = useQueryClient();
  const [, setLocation] = useLocation();
  
  // Force le mode création si l'URL contient "new"
  const isNewEditorial = id === 'new';
  
  console.log("Mode détecté:", isNewEditorial ? "CRÉATION" : "ÉDITION", "avec ID:", id);
  ...
  // Récupération des données de l'élément éditorial existant
  // Ne pas exécuter la requête si on est en mode création
  const { data: editorial, isLoading, error } = useQuery({
    queryKey: [`/api/editorials/${id}`],
    enabled: !isNewEditorial && !!id && id !== 'new', // S'assurer que nous ne chargeons pas pour 'new'
    queryFn: async () => {
      if (isNewEditorial || !id || id === 'new') {
        return null; // Retourner null pour un nouvel élément
      }
  ...
  // Effet pour initialiser le formulaire lorsque les données sont disponibles
  useEffect(() => {
    if (editorial) {
      ...
      if (editorial.imageUrl) {
        let processedUrl = editorial.imageUrl;
        if (!processedUrl.startsWith('http://') && !processedUrl.startsWith('https://')) {
          const cleanPath = processedUrl.startsWith('/') ? processedUrl.substring(1) : processedUrl;
          processedUrl = `/${cleanPath}`;
        }
  ...
  useEffect(() => {
    if (error) {
      setFormStatus({
        message: "Erreur lors du chargement de l'élément éditorial.",
        isError: true
      });
    }
  }, [error]);
  ...
  // Mutation pour mettre à jour un élément éditorial existant
  const updateMutation = useMutation({
    mutationFn: async (data: EditorialFormValues) => {
      console.log("Mise à jour avec les données:", data, "pour l'ID:", id);
      
      // Si pas de référence, on en génère une
      let reference = data.reference;
      if (!reference) {
        reference = `EDITO_${data.name.replace(/\s+/g, '_').toUpperCase()}_${Date.now().toString().slice(-6)}`;
      }
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error("Erreur serveur lors de la mise à jour:", errorData);
    ...
    onSuccess: () => {
      setFormStatus({
        message: "Élément éditorial mis à jour avec succès!",
        isError: false
      });
      queryClient.invalidateQueries({ queryKey: [`/api/editorials/${id}`] });
      queryClient.invalidateQueries({ queryKey: ['/api/editorials'] });
    },
    onError: (error: Error) => {
      setFormStatus({
  ...
  const onSubmit = async (data: EditorialFormValues) => {
    ...
    const forceCreation = id === 'new' || isNewEditorial;
    
    if (forceCreation) {
      console.log("Mode création détecté, création d'un nouvel élément");
      // Pour un nouvel élément, on utilise la mutation de création
      createMutation.mutate(data);
    } else {
      // Pour une mise à jour, on s'assure que l'ID existe avant de soumettre
      if (id && !isNaN(parseInt(id))) {
        console.log("Mode édition détecté, mise à jour de l'élément", id);
        updateMutation.mutate(data);
      } else {
        console.error("ID invalide:", id);
        setFormStatus({
          message: "Erreur: ID invalide pour la mise à jour",
          isError: true
        });
      }
    }
  ...
  return (
    <AdminLayout>
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-mono font-normal">
              {isNewEditorial ? 'Nouvel Élément Éditorial' : `Modifier: ${editorial?.name}`}
            </h1>
            <p className="text-gray-600 font-mono text-xs">
              {isNewEditorial ? 'Créer un nouvel élément visuel pour la grille' : 'Modifier les informations de cet élément éditorial'}
            </p>
          </div>
          <button
      ...
      {isLoading && !isNewEditorial && (
        <div className="bg-white border border-gray-200 p-4 mb-6">
          <p className="text-center font-mono text-sm">Chargement des données...</p>
        </div>
      )}
      
      {/* Message d'erreur de chargement */}
      {error && !isNewEditorial && (
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 mb-6 font-mono text-xs">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {/* Champ de référence supprimé - généré automatiquement */}
          ...
          <div>
            ...
            <div className="mb-4 border border-blue-200 p-3 bg-blue-50">
              <label className="block font-mono text-xs text-gray-700 mb-1">
                Uploader une nouvelle image
              </label>
              <FileUploader 
                onFileUploaded={(filePath: string) => {
                  setValue('imageUrl', filePath, { shouldValidate: true });
                  setFormStatus({
                ...
                acceptedTypes="image/*"
                targetFolder="Yhom_Editorial"
                label="Choisir une image éditorialе"
              />
            </div>
